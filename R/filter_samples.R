...
sample_order <- dplyr::group_by(
  ...
  dplyr::select(sample_id, date_taken, year, season, location_id) %>%
  unique() %>%
  dplyr::mutate(
    sample_order = order(date_taken, decreasing = FALSE)
  )
...
data <- filter(data, max_year == options$classification_year)
}
return(data)
}
