...
data <- data %>% filter(year >= 2019 | parameter == "PAC")
```
...
# 207458 (temp), 207474 (long), 545145 (slope -9)
...
nmp_diatoms <- filter(nmp, !is.na(`REG PURPOSE-DIAT` ))
 ...
 nmp_inverts <- filter(nmp, !is.na(`REG PURPOSE-INV` ))
  nmp_diatoms$`REG Mon Reason` <-  nmp_diatoms$`REG PURPOSE-INV`
 nmp_inverts <- nmp_diatoms[!duplicated(nmp_diatoms$`Sample Point code`), ] 
...
```{r}
 ...
 write.csv(results_filtered, 
           file = "2024-classification-final-v2.csv", 
           row.names = FALSE)

write.csv(diatoms_reportable, 
          file = "2024-classification-diatoms-final-v2.csv", 
          row.names = FALSE)

write.csv(inverts_reportable, 
          file = "2024-classification-inverts-final-v2.csv", 
          row.names = FALSE)
