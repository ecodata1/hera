...
```{r}
user <- Sys.info()["user"]
user_path <- "/OneDrive - Scottish Environment Protection Agency/DNA Data/"
# user_path <- "/OneDrive - Scottish Environment Protection Agency/Ecology All Staff - DNA Data/"
...
path <- paste0(
  ...
  user,
  user_path,
  "Readiness Data/classification/latest-lims-data.csv"
)
...
path <- paste0(
  ...
  user,
  user_path,
  "Readiness Data/analytical-data/all_data_clean.arrow"
)
...
path <- paste0(
  ...
  user,
  user_path,
  "Readiness Data/analytical-data/ALK_RESULTS_CHEM_SEP19.xlsx"
)
...
predictors <- dplyr::arrange(predictors, dplyr::desc(date))

### Use sampling_point layer if available ###

# Take up to the first two most recent rows from each location
 #  predictors <- slice(predictors, .by = location_id, 1:2)
# For each location, if only one row - use that row
  # predictors <- purrr::map_df(split(predictors,predictors$location_id),
  # function(location) {
  #     if(nrow(location) == 1) {
  #       return(location)
  #     }
# If two rows, return first row if date not 2026-03-16
    #   if(location$date[1] != "2026-03-16") {
    #     return(location[1, ])
    #   } else {
    #     return(location[2, ])
    #   }
   ...
   # 5187
    # predictors <- arrange(predictors, date)
    # predictors <- slice(predictors, .by = location_id, 1) 

#### Use old slope change ####
...
#         return(location)
#       } 
# # If first row date is 2026-03-16 and slope < 1 - replace slope value with value from value from most recent sampling_point sourced value. 
#         if(location$slope[1] < 1) {
#           slope <-   location$slope[1]
#           location$slope[1] <- location$slope[location$predictor_source == "sampling_point"][1]
#           location$comments[1] <- "RICT layer slope (<1) not used, replaced with previous slope value"
#           if(is.na(location$slope[1])) {
#             location$slope[1] <- slope
#              location$comments[1] <- "RICT layer slope < 1 - replace with map measured slope"
#           }
#           
#         }
#         return(location)
...
# # For each location, if only one row - use that row
 ...
 #  function(location) {
 # 
 #    # if(unique(location$location_id) %in% 10003) {
 #    #   browser()
 #    # }
 #  
 #    
 #      if(nrow(location) == 1 && location$predictor_source[1] == "sampling_point") {
 #        return(location)
 #      }
 #    
 #       if(nrow(location) == 1 && is.na(location$altitude)) {
 #        return(location)
 #      }
# If two rows, return first row if date not 2026-03-16
   #   if(location$date[1] == "2026-03-16") {
...
# # If first row date is 2026-03-16 and altitude < 5 - replace altitude value with value from value from most recent sampling_point sourced value. 
      #   if(location$altitude[1] < 5 &&
      #      !is.na(location$altitude[location$predictor_source == "sampling_point"][1]) &&
      #      location$altitude[1] < location$altitude[location$predictor_source == "sampling_point"][1]) {
      #       location$altitude[1] <- 
      #       location$altitude[location$predictor_source == "sampling_point"][1]
      #     location$comments[1] <- paste0(location$comments[1], 
      #       "RICT layer altitude (<=5) not used, replaced with previous altitude value")
      #   
      #   }
      #   if(location$altitude[1] < 5) {
      #        location$comments[1] <-  paste0(location$comments[1],
      #          "RICT layer altitude <= 5 - replace with map measured altitude")
      #   }
      # }
...
# predictors$altitude[predictors$location_id == 4411][1] <- 5
# predictors$comments[predictors$location_id == 4411][1] <- "RICT layer altitude replaced with map measured altitude"
#     
...
predictors$slope[predictors$slope < 1 & !is.na(predictors$slope)] <- 1
...
# data <- mutate(data, 
#                label = ifelse(label == "Karayevia sp", "Karayevia sp.", label))
# data <- mutate(data, 
#                label = ifelse(
#                  label == "Nitzschia dissipata subsp. media",
#                  "Nitzschia dissipata var. media", label))
# data <- mutate(data, 
#                label = ifelse(label == "Gomphonema micopus", 
#                               "Gomphonema micropus", label))
# 
# data <- mutate(data, 
#                label = ifelse(label == "Aneumastus", 
#                               "Aneumastus", label))
...
data <- filter(data, purpose %in% c("SCHED", "Planned","SCHED_ALIQ", "SCHEDULED"))
# In case 'data' variable gets written over by other scripts
test_data <- data
results <- map_df(c(2020, 2021, 2022, 2023, 2024, 2025), function(class_year) {
...
path <- paste0(
  ...
  user,
  user_path,
  "Readiness Data/classification/NMP-2025.xlsx"
)

nmp <- readxl::read_xlsx(
  path =
    path, sheet = "NMP-2025", skip = 1
)
...
nmp <- select(nmp,
  ...
  "mon_purp" = `Monitoring Purpose`,
  DESC = `Sampling Point Description`,
  "Local office" = "EAU Team"
)

diatoms <- nmp[nmp$analysis %in% c("Diat","DIAT"), ]
diatoms$analysis <- "Phytobenthos (diatoms) - Rivers"
inverts <- nmp[nmp$analysis %in% c("TL2","TL5","TL4"), ]
...
path <- paste0(
  ...
  user,
  user_path,
  "Readiness Data/classification/classification-latest-published.xlsx"
)
...
Only include locations with data from 2021, 2022, 2023, 2024, 2025.
...
results_filtered$check[results_filtered$check == FALSE] <-
  grepl("2025", results_filtered$`Years included`[results_filtered$check == FALSE])
...
path <- paste0(
  ...
  user,
  user_path,
  "Readiness Data/classification/replocs-live.csv"
)
...
replocs <- filter(replocs, CLASSIFICATION_PARAMETER %in% c("3952", "3968"))
replocs <- select(
  replocs,
  LOCATION_CODE,
...
```{r}
# only update samples taken in last year 
 results_filtered <- results_filtered[grepl("2025", results_filtered$`Years included`), ]
 
# remove investigative
# keep mon_purp where NA for now.
 
  results_filtered <- results_filtered[results_filtered$mon_purp == "Understanding the environment" | is.na(results_filtered$mon_purp), ]
...
```{r}
user <- Sys.info()["user"]
folder <- "Readiness Data/classification/"
file_path <- paste0(
  "C:/Users/",
  user,
  user_path,
...
diatoms_path <- paste0(
  file_path,
  "2025-classification-diatoms-final.csv"
)
...
inverts_path <- paste0(
  file_path,
  "2025-classification-inverts-final.csv"
)
...
samples_path <- paste0(
  file_path,
  "sample-classification-2025.csv"
)
