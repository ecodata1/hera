...
Install the development version from [GitHub](https://github.com/ecodata1/hera) with:
...
```{r steps, validation, warning=F, message=F}
library(hera)
...
```{r steps, indices, warning=F, message=F}
...
```{r predictions, warning=F, message=F}
...
```{r assessments, warning=F, message=F}
