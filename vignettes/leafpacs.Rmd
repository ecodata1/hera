...
A dataframe containing all the variables required for assessment.
...
questions <- questions %>% filter(parameter == "River Macrophytes")
...
Dataframe of indices used in this assessment.
...
indices_function <- function(data) {
  ...
  algae <- data %>%
    group_by(.data$cover) %>%
    summarise(
      n = n_distinct(.data$label[.data$algal.taxon == "Y"])
    )
  ...
  data <- data %>% summarise(
    ...
    rn_a_taxa = n_distinct(.data$label[.data$aquatic.taxon == "Y"]),
...
indexes <- indices_function(questions)
head(indexes)
```
...
```{r prediction}
prediction <- function(predictors, reference_algae = 0.05) {
...
if(any(names(data)%in% "result_id")) {
  sample_ids %>% select(-result_id)
}
sample_ids <- sample_ids %>% unique()

indexes <- right_join(sample_ids, indexes,
  by = c("sample_id" = "sample_id")
)

predictions <- right_join(sample_ids, predictions,
  by = c("location_id" = "location_id")
)

data <- bind_rows(questions, predictions, indexes)
```
...
assessment_function <- function(data, assessment_table = NULL) {
  ...
  if (length(data$question[data$question == "rmni"]) == 0) {
    return(NULL)
  }
...
assessments <- assessment_function(data, assessment_table)
assessments
```
...
confidence_function <- function(data) {
  ...
  data <- data %>% select(1, high, good, moderate, poor, bad)
...
# 
# assessments <- right_join(sample_ids, assessments,
#   by = c("sample_id" = "sample_id")
# )
# 
# data <- bind_rows(data, assessments)
...
#   data[data$question =="eqr", ],
#   aggregation_variables <- c("water_body_id", "year", "parameter")
# )
# 
# confidences <- hera:::confidences(data = aggregates, confidence_function)
# water_body_ids <- sample_ids(-sample_id, )
# confidences <- inner_join(aggregates, confidences, by = "water_body_id")
# 
# 
# 
...
# all_data <- all_data  %>% dplyr::filter(parameter == "River Macrophytes")
