...
-   Difficult because sites not always sampled every year. Need a solution- baseline only sites?
...
-   Convert() function does a lot of tweaks to question/REPORTED_NAME/parameter/analysis - can these go in a .csv lookup file? Is paramter the right term?
...
predictors <- utils::read.csv(system.file("extdat",
 ...
 predictors <- predictors[predictors$date != "", ]
...
# review <- filtered_data %>%
#   filter(parameter != "PAC")  %>%
#   group_by(location_id, parameter) %>% 
#   summarise(years = length(unique(year))
#         )
# 
# write.csv(review, file = "vignettes/data/review.csv")
# 
# 
# review <- data %>%
#   filter(parameter != "PAC")  %>%
...
write.csv(data, file="vignettes/data/classification_input_data.csv")
# test_data <- data[data$location_id == "101932",] 
 # test <- data[data$location_id == "122748",] 
...
write.csv(results, file="vignettes/data/raw-classification-results.csv")
```
