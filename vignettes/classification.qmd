...
```{r}
 ...
 lims_eco <- readr::read_csv(
  "vignettes//data//2024-03-21-lims-data.csv"
 )
...
missing_alk \<- data\[!is.na(data\$chemistry_site) &

is.na(data\$alkalinity) &

data\$analysis_repname %in% unique( data\$analysis_repname)

, \]

missing_alk \<- missing_alk\[missing_alk\$analysis_repname %in% c("Diatom Taxa", "Invert Taxa Family Lab", "RIVER_DIATOMS" , "LAB_INVERTS" ),\]
...
test_data <- data[is.na(data$grid_reference), ]
# 350436
...
results <- map_df(c(2021,2022,2023), function(class_year) {
  ...
  # filter_data <- filter_data[filter_data$location_id == "110106",]
...
# write.csv(results, file = "vignettes\\data\\sample-classification-2024.csv", row.names = FALSE)
