...
input <- tibble(
  ...
  data_type = c("character", "character",
           "character", "number",
           "number", "character", 
           "number",  "number"),
...
assessment_function <- function(data) {
  ...
  data <- bind_rows(sample, location)
  data$parameter <- "Phytobenthos (diatoms)"
  data <-  mutate(data,
