...
predictors$sample_id[1:2] <- 1234
predictors$location_id[1:2] <- 8715
predictors$location_id[3:4] <- 1
...
# All Together Now
# Combine observations, metrics & predictions
sample_info <- data %>%
...
predictions <- inner_join(sample_info, predictions,
  by = c("location_id","sample_id")
)
...
assessment_function <- function(data, assessment_table) {
  ...
  data <- data %>%
    ...
    filter(.data$question %in% c("Reference ASPT", "WHPT_ASPT"))
  ...
  data$eqr <- data$WHPT_ASPT / data$`Reference ASPT`

  class <- cut(data$eqr,
    breaks = c(1, assessment_table$value),
    labels = assessment_table$assessment
  )
...
# No need to edit this code
launch_app(new_catalogue = catalogue, data = whpt::demo_data)
```
