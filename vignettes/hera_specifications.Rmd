...
Based on the predicted and observed indices - we assess each sample using
`assessment()`. The 'assessment' stage includes any type of test or categorisation of our data. This could be WFD classification, compliance check, bathing water or discharge assessment. Broadly, any type of grading or assessment against some type of expected value. 
...
A custom report that compiles, groups and presents one or more assessment output can be created. For instance running and presentating a number of WFD assessments, grouping by water body and hierarchy for a River Basin Management Plan:

```
# TODO!
report(demo_data, 'rbmp') 
...
potential pressures. We use the `report()` function to build a custom report to help diagnose potential
pressures.

```
# TODO!
rerport(demo_data, 'diagnose_wfd_pressures') %>% 
...
To facilitate general data exploration, an `aggregate` herlper function can group the outputs by season, year, multi-year or water body as
required:
...
```
# TODO!
report(demo_data, trends, scenario="wfd_forecast") %>%  
head()

report(demo_data, measures, scenario="measure_impact") %>% 
head()
