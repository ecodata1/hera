...
Below, we breakdown classification into independent steps. Each step resprents a
function within the hera package. This allows the code and rules between
...
The Europe Environment Agency as prodcued a data dictionary for reporting. However this is mainly for high-level reporting. We use some aspects of thi standard here to aid onward reporting to EEA.


### What does this look like?

A small sample demo dataset `demo_input` contains diatoms, macrophytes and inverts quality elements. 
...
```{r}
hera::demo_input %>% select(location_id, sample_id, quality_element) %>% 
  head(5)
...
An observation consists of three variables `question`, `response` and
`observation_id`. The `observation_type` uniquely  identifies the type of
observation made. The `question` variable identifies the what is being
determined such as `alkalinity` or `depth` etc. And the `response` is the value
...
```
demo_data %>% select(observation_id, question, response)
...
```
demo_data %>% select(taxon, question, response)
...
* Validation Rules
* WFD Indices, Model and Environmental Standards
* Taxon Table
* EQR Boundaries
* WFD Parameter Hierarchy
