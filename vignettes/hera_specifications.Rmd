...
`classification()`. The 'compliance' stage includes any type of test or assessment of our data. This could be WFD classification, compliance check, bathing water or discharge assessment. Really any type of grading or assessment against some type of expected value. 
...
Note, classification results for inverts are missing - this has not be
implemented yet.
...
existing datasets and *ad hoc* data, a third column `taxon` is useful due to the
...
```{r}
demo_data %>%  select(`mean_alkalinity`, `grid_reference`, `season`)
```
...
```{r, echo=TRUE}
environment_agency_data <- hera:::ead(site_id = 43378)
...
tidyr::unnest(predictions, cols = c(prediction)) %>%
  select(date_taken, index, predicted_response) %>%
  unique()
