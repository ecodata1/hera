...
community such as [ropenSci](https://ropensci.org/).
...
Based on the predicted and observed indices - we classify each sample using
`classification()`:
...
There are a number of data dictionaries, metadata standards and semantic data
definition across disciplines and organisations. It's unclear exactly how we
define out input and output datasets. We propose using best practice and make
strides towards using common definition and standards where possible. In a
similarly way to the climate model communities use of the [Climate and Forecast
Standard Names](http://cfconventions.org/index.html).

The Europe Environment Agency has produced a data dictionary for reporting.
...
head(environment_agency_data[, 1:6])
